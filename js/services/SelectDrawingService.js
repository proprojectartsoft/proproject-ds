angular.module($APP.name).factory('SelectDrawingService', ["$document", "$compile", "$rootScope", "$controller", "$timeout",
    function ($document, $compile, $rootScope, $controller, $timeout) {
        var defaults = {
            ...
            backdropCancel: true,
            footerTemplate: null,
            modalClass: "modal de-selectdrawing",
            css: {
                'margin': '0 auto',
                'max-height': '100%',
                'overflow': 'auto'
        ...
        return function Dialog(templateUrl, options, passedInLocals) {
            ...
            var modalEl = angular.element(
                    ...
                    '      <div class="modal-header" style="border:none;padding:0px;"><h2>Select drawing</h2>' +
                    '           <span class="de-text pull-right">Your computer</span>' +
                    '      </div>' +
                    modalBody +
                    '      <div class="modal-footer">' +
                    '           <div class="pull-right"><button class="de-btn de-red">Cancel</button><button class="de-btn de-green">Save</button></div>' +
                    '      </div>' +
                    '    </div>' +
                    '  </div>' +
                    '</div>');
