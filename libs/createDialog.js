angular.module('fundoo.services', []).factory('createDialog', ["$document", "$compile", "$rootScope", "$controller", "$timeout",
    function ($document, $compile, $rootScope, $controller, $timeout) {
        ...
        return function Dialog(templateUrl/*optional*/, options, passedInLocals) {
            ...
            console.log(passedInLocals)
            if (passedInLocals.selectedImg) {
                $rootScope.selectedImg = passedInLocals.selectedImg;
            }
            ...
            var modalBody = (function () {
                if (options.template) {
                    if (angular.isString(options.template)) {
                        ...
                        return '<div class="modal-body" style=" padding-top: 0px;">' + options.template + '</div>';
                    } else {
                        // jQuery/JQlite wrapped object
                        return '<div class="modal-body" style=" padding-top: 0px;">' + options.template.html() + '</div>';
                    }
                } else {
                    // Template url
                    return '<div class="modal-body" style=" padding-top: 0px;" ng-include="\'' + options.templateUrl + '\'"></div>'
                }
            })();
            ...
            var modalEl = angular.element(
                    ...
                    '        <button type="button" class="close" style="    padding: 10px 15px 10px;outline:none" ng-click="$modalCancel()">&times;</button>' +
                    '        <h1>' + passedInLocals.title + '</h1>' +
                    '      </div>' +
                    modalBody +
                    '    </div>' +
                    '  </div>' +
